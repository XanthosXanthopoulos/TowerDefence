...
int main(int argc, char *argv[])
{
	...
	glm::vec2 live_mouse_position(0);
	...
	while (quit == false)
	{
		...
		while (SDL_PollEvent(&event))
		{
			...
			else if (event.type == SDL_KEYDOWN)
			{
				...
				{
					renderer->ToggleTowerMode();
				}
				else if (event.key.keysym.sym == SDLK_m)
				{
					renderer->ToggleLightningTowerMode();
				}
				else if (event.key.keysym.sym == SDLK_d)
				{
					renderer->ToogleDemolishMode();
				}
			...
			else if (event.type == SDL_MOUSEMOTION)
			{
				...
				live_mouse_position = glm::vec2(x, y);
		...
		// Update
		renderer->Update(dt, live_mouse_position);
