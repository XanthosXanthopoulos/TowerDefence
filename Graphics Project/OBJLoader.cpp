...
inline void OBJLoader::read_vertex(const char* buff)
{
	...
	sumTangents.push_back(glm::vec3(0));
	sumBitangents.push_back(glm::vec3(0));
}
...
void OBJLoader::generateDataFromFaces()
{
	...
	mesh->tangents.reserve(shared_faces.size() * 3);
	mesh->bitangents.reserve(shared_faces.size() * 3);
	...
	for (int face = 0; face < shared_faces.size(); face++)
	{
		...
		glm::vec3 p0 = shared_vertices[shared_faces[face].vertices.x];
		glm::vec3 p1 = shared_vertices[shared_faces[face].vertices.y];
		glm::vec3 p2 = shared_vertices[shared_faces[face].vertices.z];
		...
		if (glm::any(glm::greaterThanEqual(shared_faces[face].normals, glm::ivec3(0))))
		{
			if (glm::any(glm::greaterThanEqual(shared_faces[face].texcoords, glm::ivec3(0))))
			{
				...
				glm::vec2 t0 = shared_textcoord[shared_faces[face].texcoords.x];
				glm::vec2 t1 = shared_textcoord[shared_faces[face].texcoords.y];
				glm::vec2 t2 = shared_textcoord[shared_faces[face].texcoords.z];
				...
				glm::vec3 bitangent;
				bitangent.x = r * (-deltaPos1.x * deltaUv2.x + deltaPos2.x * deltaUv1.x);
				bitangent.y = r * (-deltaPos1.y * deltaUv2.x + deltaPos2.y * deltaUv1.x);
				bitangent.z = r * (-deltaPos1.z * deltaUv2.x + deltaPos2.z * deltaUv1.x);

				sumTangents[shared_faces[face].vertices.x] += glm::normalize(tangent);
				sumTangents[shared_faces[face].vertices.y] += glm::normalize(tangent);
				sumTangents[shared_faces[face].vertices.z] += glm::normalize(tangent);

				sumBitangents[shared_faces[face].vertices.x] += glm::normalize(bitangent);
				sumBitangents[shared_faces[face].vertices.y] += glm::normalize(bitangent);
				sumBitangents[shared_faces[face].vertices.z] += glm::normalize(bitangent);
	...
	{		
		...
		if (glm::any(glm::greaterThanEqual(shared_faces[face].normals, glm::ivec3(0))))
		{
			...
			if (glm::any(glm::greaterThanEqual(shared_faces[face].texcoords, glm::ivec3(0))))
			{
				...
				mesh->tangents.push_back(glm::normalize(sumTangents[shared_faces[face].vertices.x]));
				mesh->tangents.push_back(glm::normalize(sumTangents[shared_faces[face].vertices.y]));
				mesh->tangents.push_back(glm::normalize(sumTangents[shared_faces[face].vertices.z]));

				mesh->bitangents.push_back(glm::normalize(sumBitangents[shared_faces[face].vertices.x]));
				mesh->bitangents.push_back(glm::normalize(sumBitangents[shared_faces[face].vertices.y]));
				mesh->bitangents.push_back(glm::normalize(sumBitangents[shared_faces[face].vertices.z]));
