...
GLuint TextureManager::RequestCubeMap(std::vector<std::string> filenames, bool hasMipmaps)
{
	GLuint cubeMapId = 0;
	...
	{
		SDL_Surface* surf = IMG_Load(filenames[i].c_str());
		if (surf == 0)
		{
			printf("Could not Load texture %s\n", filenames[i]);
			printf("SDL load Error %s\n", SDL_GetError());
		...
		TextureContainer container;
		container.filename = filenames[i];
		container.hasMipmaps = hasMipmaps;
		GLenum texture_format;
		GLint nOfColors;
		...
		SDL_LockSurface(surf);
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, nOfColors, surf->w, surf->h, 0, texture_format, GL_UNSIGNED_BYTE, data);
		SDL_UnlockSurface(surf);
		delete[] data;
		if (surf) SDL_FreeSurface(surf);
	}
